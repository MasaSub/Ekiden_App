...
st.markdown("""
    ...
    div.stButton > button[kind="primary"] {
        background-color: #FF4B4B; color: white; border: 2px solid #ff9999;
    }
    ...
    div.stButton > button[kind="secondary"]:hover {
        background-color: #444; border-color: #888; color: white;
    }
...
def initialize_race(race_name, section_count, teams_dict, main_team_id):
    ...
    sh.worksheet(WORKSHEET_CONFIG).append_rows(config_data)
    st.cache_data.clear()
...
# ==========================================
    ...
    # -------------------------------------
        ...
        selected_tid = st.selectbox(
            ...
            index=team_ids_ordered.index(st.session_state["watch_tid"]) if st.session_state["watch_tid"] in team_ids_ordered else 0
        )
        st.session_state["watch_tid"] = selected_tid # 選択状態を保持
        ...
        if t_df.empty:
            ...
            # 【追加提案】ペース表示 (直近の1kmラップから推定)
            try:
                # 文字列 "MM:SS.f" を秒に変換する簡易ロジック
                def str_to_sec(s):
                ...
                # ペース表示 (Relay地点などは正確な1kmではないが目安として表示)
            ...
            # 自分のインデックスを探す
            my_idx = loc_df[loc_df['TeamID'] == selected_tid].index[0]
            ...
            st.write("📝 通過履歴")
            # 必要な列だけ見やすく表示
            history_df = t_df[['Section', 'Location', 'Split', 'KM-Lap', 'Rank']].iloc[::-1] # 新しい順
